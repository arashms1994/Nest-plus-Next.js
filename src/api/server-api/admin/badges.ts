...
export const createBadge = async (body: Partial<IBadge>) => {
  const data = await apiFetch(`${ADMIN_BASE_URL}/badges`, {
    method: "post",
...
export const updateBadge = async (id: string, body: any) => {
  const data = await apiFetch(`${ADMIN_BASE_URL}/badges/${id}`, {
    method: "post",
    body: JSON.stringify(body),
  });
...
export const deleteBadge = async (id: string): Promise<unknown> => {
  const res = await apiFetch(`${ADMIN_BASE_URL}/badges/${id}`, {
    method: "delete",
  });
  return res;
};
...
export const getBadgeById = async (id: string) => {
  const data = await apiFetch<IBadge>(`${ADMIN_BASE_URL}/badges/${id}`, {
    cache: "force-cache",
    next: {
